...
export default function ReceiveDialog({ asaID }: QRCodeProps) {
  ...
  const onScan = async (scannedJson: string) => {
    if (scannedJson) {
      ...
      try {
        [publicKey, privateKey] = keyStore.getAccountKeys(asaID, securePrompt("Tell my your secret:"));
      } catch (e) { setOpen(false); error("Wrong secret for private key!"); return; }

      try {
        payload = JSON.parse(scannedJson) as QrCodeData;
        if (!payload.amount || !payload.publicKey) throw Error();
      } catch (e) { error("Invalid qr code!"); return;}

      const transactionTime = new Date().getTime();

      paymentService.sendAsa(publicKey!, payload!.publicKey, Number(asaID), Number(payload!.amount), privateKey!)
        .then(response => success(`Transaction approved! - ${(((new Date()).getTime() - transactionTime) / 1000).toFixed(2)} sec`))
    ...
    return (
      <Box>
        <Button startIcon={<SendIcon/>} color="secondary" variant="contained" onClick={handleClickOpen}>
          Pay
      </Button>
