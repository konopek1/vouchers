    ...
    async run(n: number): Promise<any[]> {
        ...
        for (let i = 0; i < n; i++) {
            ...
            const comp = this.computation()
                .then((result) => ({ result, i: this.results.push({ time: process.hrtime(start), error: false }) }))
                .catch((result) => ({ result: JSON.stringify(result), i: this.results.push({ time: process.hrtime(start), error: true }) }))
